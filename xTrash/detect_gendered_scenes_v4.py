...
class ScenePatternDetector:
    def __init__(self):
        ...
        # Charger YOLOv8 pour détection d'objets
        print("Chargement du modèle YOLOv8...")
        # Utiliser yolov8s (small) pour meilleure détection, ou yolov8m si performance OK
        self.yolo_model = YOLO('yolov8s.pt')  # Version small pour meilleur équilibre
        print("YOLOv8 chargé!")
        
        # Charger BLIP pour l'analyse visuelle complémentaire
    ...
    def generate_description(self, image):
        ...
        try:
            ...
            print(f"Erreur description image: {e}")
            return ""
    ...
    def analyze_film(self, video_path, interval_seconds=2, start_time_s=0, end_time_s=None, output_prefix=None):
        ...
        for i, frame_data in enumerate(frames_data):
            ...
            # Description visuelle complémentaire avec BLIP
            description = self.generate_description(frame_data['image'])
        ...
        # Construire une chronologie fiable basée sur les plans
        start_ms = start_time_s * 1000
    ...
    def detect_solitude(self, frames_analysis, criteria):
        """Détecte les scènes de solitude"""
        scenes = []
        current_scene = None
        
        for frame in frames_analysis:
            # Vérifier si une seule personne est visible
            single_person = any(word in frame['description_fr'].lower() for word in ['person', 'man', 'woman', 'homme', 'femme']) and \
                          not any(word in frame['description_fr'].lower() for word in ['people', 'group', 'gens', 'personnes'])
