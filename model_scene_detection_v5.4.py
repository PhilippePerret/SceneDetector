...
def extract_color_palette(frame_bgr, n_colors=5):
    """Extrait la palette de couleurs dominantes avec K-means"""
    ...
    if w > 320:
        scale = 320 / w
        new_h = int(h * scale)
        frame_small = cv2.resize(frame_bgr, (320, new_h))
    else:
    ...
    # Récupérer les couleurs dominantes et leurs proportions
    ...
    for i in range(n_colors):
        proportion = (labels == i).sum() / len(labels)
...
def compare_color_palettes(palette1, palette2):
    """Compare deux palettes de couleurs"""
    if not palette1 or not palette2:
        return 0.0
    
    # Calculer la distance moyenne entre les couleurs les plus proches
    colors1 = np.array(palette1['colors'])
    colors2 = np.array(palette2['colors'])
    
    total_distance = 0
    for c1, p1 in zip(colors1, palette1['proportions']):
        # Trouver la couleur la plus proche dans palette2
        distances = np.sqrt(np.sum((colors2 - c1) ** 2, axis=1))
        min_dist = np.min(distances)
        total_distance += min_dist * p1
    
    # Normaliser (distance max théorique = 441.67 pour RGB)
    similarity = 1.0 - (total_distance / 441.67)
    return max(0, similarity)

def group_shots_into_scenes(shots, frames_analysis):
    """VRAIE fonction pour regrouper les plans en scènes basé sur la continuité"""
    if not shots or not frames_analysis:
        return shots
    
    # Créer un index frame_time -> frame_data pour accès rapide
    frame_index = {}
    for frame in frames_analysis:
        frame_index[frame['time_ms']] = frame
    
    # Analyser chaque plan
    shot_features = []
    for start_ms, end_ms, mid_ms in shots:
        # Prendre quelques frames du plan pour analyse
        features = {
            'start_ms': start_ms,
            'end_ms': end_ms,
            'objects': set(),
        ...
        # Collecter les features de ce plan
        for frame in frames_analysis:
            if start_ms <= frame['time_ms'] <= end_ms:
                # Objets détectés
                for obj in frame.get('objects', []):
                    features['objects'].add(obj['class'])
                    if obj['class'] == 'person':
                        features['persons'] = max(features['persons'], frame.get('person_count', 0))
                
                # Couleurs et ambiance (prendre celle du milieu)
                if abs(frame['time_ms'] - mid_ms) < 1000:  # Frame proche du milieu
                    features['colors'] = frame.get('color_palette')
                    features['ambient'] = frame.get('ambient_light')
        
        shot_features.append(features)
    
    # Regrouper les plans similaires
    scenes = []
    current_scene = None
    shot_signatures = []  # Stocker les signatures visuelles de chaque plan
    
    # Calculer une signature simple pour chaque plan (pour détecter les répétitions)
    for features in shot_features:
        signature = {
            'objects': frozenset(features['objects']),  # Ensemble d'objets
            'persons': features['persons'],
            'colors': tuple(features['colors']['colors'][0]) if features['colors'] else None  # Couleur dominante
        }
        shot_signatures.append(signature)
    
    for i, features in enumerate(shot_features):
        if current_scene is None:
            # Commencer nouvelle scène
            current_scene = {
                'start_ms': features['start_ms'],
                'end_ms': features['end_ms'],
                'shots': [i],
                'common_objects': features['objects'].copy(),
                'persons': features['persons'],
                'base_colors': features['colors'],
                'base_ambient': features['ambient']
            }
        else:
            # RÈGLE 1 : Vérifier si c'est un plan déjà vu = MÊME SCÈNE AUTOMATIQUEMENT
            is_repeated_shot = False
            for prev_idx in current_scene['shots']:
                if prev_idx < len(shot_signatures):
                    prev_sig = shot_signatures[prev_idx]
                    curr_sig = shot_signatures[i]
                    
                    # Mêmes objets ET même nombre de personnes = PLAN RÉPÉTÉ
                    if (prev_sig['objects'] == curr_sig['objects'] and 
                        prev_sig['persons'] == curr_sig['persons']):
                        is_repeated_shot = True
                        print(f"  PLAN RÉPÉTÉ détecté à {features['start_ms']/1000:.0f}s (= plan {prev_idx+1})")
                        break
            
            if is_repeated_shot:
                # MÊME PLAN = MÊME SCÈNE, pas de question !
                current_scene['end_ms'] = features['end_ms']
                current_scene['shots'].append(i)
                current_scene['common_objects'] = current_scene['common_objects'].union(features['objects']) if features['objects'] else current_scene['common_objects']
                current_scene['persons'] = max(current_scene['persons'], features['persons'])
            else:
                # Sinon, vérifier la similarité normale
                similarity_score = 0
                total_checks = 0
                
                # 1. Objets communs (40% du score)
                if features['objects'] and current_scene['common_objects']:
                    common = features['objects'].intersection(current_scene['common_objects'])
                    obj_similarity = len(common) / max(len(features['objects']), len(current_scene['common_objects']))
                    
                    # Bonus si personnes présentes des deux côtés
                    if 'person' in features['objects'] and 'person' in current_scene['common_objects']:
                        obj_similarity = min(1.0, obj_similarity + 0.3)
                    
                    similarity_score += obj_similarity * 0.4
                    total_checks += 0.4
                
                # 2. Nombre de personnes similaire (20% du score)
                if features['persons'] > 0 or current_scene['persons'] > 0:
                person_diff = abs(features['persons'] - current_scene['persons'])
                person_similarity = 1.0 - min(person_diff / 3.0, 1.0)  # Max diff = 3
                similarity_score += person_similarity * 0.2
                total_checks += 0.2
            
            # 3. Couleurs similaires (30% du score)
            if features['colors'] and current_scene['base_colors']:
                color_sim = compare_color_palettes(features['colors'], current_scene['base_colors'])
                similarity_score += color_sim * 0.3
                total_checks += 0.3
            
            # 4. Ambiance lumineuse similaire (10% du score)
            if features['ambient'] and current_scene['base_ambient']:
                light_sim = 1.0 if compare_ambient_light(features['ambient'], current_scene['base_ambient']) else 0.0
                similarity_score += light_sim * 0.1
                total_checks += 0.1
            
            # Normaliser le score
            if total_checks > 0:
                similarity_score = similarity_score / total_checks
            
                # Seuil de similarité pour considérer comme même scène
                SIMILARITY_THRESHOLD = 0.4
                
                # Détection spéciale pour changement INT/EXT majeur
            # Si on passe de personnes à pas de personnes ET changement de couleurs = nouvelle scène
            drastic_change = False
            if current_scene['persons'] >= 1 and features['persons'] == 0:
                # On perd les personnes
                if features['colors'] and current_scene['base_colors']:
                    color_sim = compare_color_palettes(features['colors'], current_scene['base_colors'])
                    if color_sim < 0.3:  # Couleurs très différentes
                        drastic_change = True
                        print(f"  Changement drastique détecté à {features['start_ms']/1000:.0f}s")
            
                if similarity_score >= SIMILARITY_THRESHOLD and not drastic_change:
                # Ajouter à la scène courante
                current_scene['end_ms'] = features['end_ms']
                current_scene['shots'].append(i)
                # Mettre à jour les objets communs (union plutôt qu'intersection pour être plus permissif)
                current_scene['common_objects'] = current_scene['common_objects'].union(features['objects']) if features['objects'] else current_scene['common_objects']
                # Mettre à jour les personnes (garder le max)
                current_scene['persons'] = max(current_scene['persons'], features['persons'])
            else:
                # Vérifier si la "nouvelle scène" n'est pas trop courte (< 3 secondes)
                # Si trop courte, la fusionner avec la précédente
                scene_duration = current_scene['end_ms'] - current_scene['start_ms']
                if scene_duration < 3000 and len(scenes) > 0:  # Moins de 3 secondes
                    # Fusionner avec la scène précédente
                    prev_start, prev_end, prev_mid = scenes[-1]
                    scenes[-1] = (prev_start, features['end_ms'], (prev_start + features['end_ms']) // 2)
                    print(f"  Fusion micro-scène {current_scene['start_ms']/1000:.0f}s")
                else:
                    # Vraie nouvelle scène
                    scenes.append((current_scene['start_ms'], current_scene['end_ms'], (current_scene['start_ms'] + current_scene['end_ms']) // 2))
    ...
    print(f"Regroupement: {len(shots)} plans -> {len(scenes)} scènes")
    for i, (start_ms, end_ms, mid_ms) in enumerate(scenes):
        print(f"  Scène {i+1}: {start_ms/1000:.0f}s - {end_ms/1000:.0f}s")
    return scenes
...
class ScenePatternDetector:
    ...
    def _initialize_scene_detectors(self):
        ...
        return {
            ...
            'bar_alcool': {
                'visual_keywords': ['bar', 'pub', 'alcool', 'alcohol', 'beer', 'bière', 'whisky', 'drink', 'bottle', 'glass', 'wine'],
                'audio_keywords': ['boire', 'drink', 'beer', 'bière', 'alcohol', 'wine', 'cheers'],
                'min_duration': 10  # Réduit pour détecter plus facilement
            },
            'jeux': {
    ...
    def analyze_film(self, video_path, interval_seconds=2, start_time_s=0, end_time_s=None, output_prefix=None):
        ...
        print(f"Analyse des scènes-type: {video_path}")
        ...
        # Préparer la barre de progression améliorée
        total_frames = len(frames_data)
        print(f"\nAnalyse des frames:")
        # Afficher tous les astérisques en gris
        print(f"\033[90m{'*' * total_frames}\033[0m", end='', flush=True)
        # Revenir au début de la ligne
        print(f"\r", end='', flush=True)
        
        for i, frame_data in enumerate(frames_data):
            # Remplacer l'astérisque courant par un vert
            print(f"\033[92m*\033[0m", end='', flush=True)
            
            # Détection d'objets avec YOLO
            objects, boxes = self.detect_objects(frame_data['image'])
            
            # Extraction de l'ambiance lumineuse ET palette de couleurs
            frame_bgr = cv2.cvtColor(np.array(frame_data['image']), cv2.COLOR_RGB2BGR)
            ambient_light = extract_ambient_light(frame_bgr, boxes)
            color_palette = extract_color_palette(frame_bgr, n_colors=5)
            ...
            # Créer une description avec TOUS les objets détectés
            obj_names = [obj['class'] for obj in objects]
            description_fr = ' '.join(obj_names) if obj_names else ""
        ...
        # Détection des scènes-type
        print("Détection scènes-type: ", end='')
        detected_scenes = self.detect_model_scenes(frames_analysis)
        print()  # Nouvelle ligne après détections
        ...
        # Construire une chronologie fiable avec PySceneDetect + regroupement
        print("Détection des plans...")
        shots = detect_scenes_pyscenedetect(video_path, start_time_s, end_time_s)
        
        # REGROUPER LES PLANS EN SCÈNES (ENFIN !)
        scenes = group_shots_into_scenes(shots, frames_analysis)
        
        timeline = []
        for (s_ms, e_ms, m_ms) in scenes:  # Utiliser SCENES, pas shots !
    ...
    def detect_model_scenes(self, frames_analysis):
        """Détecte toutes les scènes-type dans le film"""
        ...
        for scene_type, criteria in self.scene_detectors.items():
            if not first:
                print(", ", end='')
            print(f"{scene_type}", end='', flush=True)
    ...
    def detect_bar_alcool(self, frames_analysis, criteria):
        """Détecte les scènes de bar avec objets YOLO"""
        scenes = []
        current_scene = None
        bar_objects = ['bottle', 'wine glass', 'cup', 'bowl']  # Objets YOLO liés aux bars
        
        for frame in frames_analysis:
            is_bar = False
            
            # Vérifier objets YOLO
            for obj in frame.get('objects', []):
                if any(bar_obj in obj['class'].lower() for bar_obj in bar_objects):
                    is_bar = True
                    break
            
            # Ou vérifier mots-clés
            if not is_bar:
                is_bar = self.check_keywords_match(
                    frame.get('description_fr', '') + ' ' + frame.get('dialogue', ''),
                    criteria['visual_keywords'] + criteria['audio_keywords']
                )
...
<head>
    ...
    <style>
        ...
        .scene-item.selected .timecode-link {{
            color: white !important;
        }}
        .scene-item.selected .scene-duration {{
            color: #ddd !important;
        }}
...
'''
        ...
        for scene in all_scenes_sorted:
            ...
            # Calculer end_seconds correctement
            end_seconds = self.timecode_to_seconds(scene.get('end_timecode', scene['start_timecode']))
            
            html += f'''
                <div class="scene-item {style_class}" data-start="{start_seconds}" data-end="{end_seconds}">
...
'''
    ...
    <script>
        ...
        // Fonction pour mettre à jour la sélection (UN SEUL élément sélectionné)
        function updateSelection() {
            const currentTime = video.currentTime;
            const allSceneItems = document.querySelectorAll('.scene-item');
            ...
            // Puis sélectionner le bon
            for (let item of allSceneItems) {
                const start = parseFloat(item.dataset.start);
                const end = parseFloat(item.dataset.end) || (start + 300);
                
                if (currentTime >= start && currentTime < end) {
                    item.classList.add('selected');
                    // Scroller seulement si nécessaire
                    if (!item.classList.contains('was-selected')) {
                        item.scrollIntoView({ behavior: 'smooth', block: 'center' });
                        // Retirer was-selected des autres
                        allSceneItems.forEach(other => {
                            if (other !== item) other.classList.remove('was-selected');
                        });
                        item.classList.add('was-selected');
                    }
                    break; // Un seul sélectionné
                }
            }
        }
...
'''
    ...
    def detect_all_film_scenes(self, frames_analysis):
        ...
        print("Détection des scènes non-typées du film...")
        ...
        last_frame = frames_analysis[-1] if frames_analysis else first_frame
        print(f"Détectées {len(scenes)} scènes non-typées (de {first_frame['timecode']} à {last_frame['timecode']})")
        return scenes
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Détection de scènes-type dans les films')
    parser.add_argument('path', nargs='?', help='Fichier vidéo ou dossier à analyser')
    parser.add_argument('--interval', type=int, default=config.get('interval', 4), help='Intervalle entre frames')
    ...
    parser.add_argument('--output', type=str, default=config.get('output'), help='Préfixe/dossier de sortie')
    ...
    if not args.path:
        print("Erreur: Aucun fichier ou dossier spécifié (ni en argument ni dans config)")
        sys.exit(1)
