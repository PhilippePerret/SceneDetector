...
# ====== Regroupement des plans en scènes (règle: MÊME PLAN = MÊME SCÈNE) ======

def _shot_signature_from_frame(frame):
    """Construit une signature compacte du plan à partir d'une frame analysée."""
    # Ensemble d'objets détectés (classes YOLO)
    obj_set = set(obj['class'] for obj in frame.get('objects', []))
    # Nombre de personnes
    persons = int(frame.get('person_count', 0) or 0)
    # Ambiance (HSV) quantifiée grossièrement
    amb = frame.get('ambient_light') or {}
    hue = int(amb.get('hue', 0))
    sat = int(amb.get('saturation', 0))
    bri = int(amb.get('brightness', 0))
    h_bin = hue // 15   # 12 bins
    s_bin = sat // 32   # 8 bins approx
    v_bin = bri // 32   # 8 bins approx
    return {
        'objects': obj_set,
        'persons': persons,
        'h': h_bin,
...
def _signature_similarity(sig_a, sig_b):
    """Score de similarité [0..1] entre deux signatures de plan."""
    # Objets (Jaccard)
    a_objs, b_objs = sig_a['objects'], sig_b['objects']
    if a_objs or b_objs:
        inter = len(a_objs & b_objs)
        union = len(a_objs | b_objs) or 1
        obj_sim = inter / union
    else:
        obj_sim = 0.0
    # Personnes (exact = 1, diff=1 => 0.6, >1 => 0.0)
    dp = abs(sig_a['persons'] - sig_b['persons'])
    if dp == 0:
        per_sim = 1.0
    elif dp == 1:
        per_sim = 0.6
        ...
        per_sim = 0.0
    # Couleurs (bins proches)
    dh = abs(sig_a['h'] - sig_b['h'])
    ds = abs(sig_a['s'] - sig_b['s'])
    dv = abs(sig_a['v'] - sig_b['v'])
    if dh <= 1 and ds <= 1 and dv <= 1:
        col_sim = 1.0
    elif dh <= 2 and ds <= 2 and dv <= 2:
        col_sim = 0.6
    else:
        col_sim = 0.0
    # Pondération
    return 0.5 * obj_sim + 0.2 * per_sim + 0.3 * col_sim

def group_shots_into_scenes(shots, frames_analysis, sim_threshold=0.5):
    """Regroupe les plans en scènes.
    Règles appliquées:
      - Même signature qu'un plan de la scène en cours => même scène (prioritaire)
      - Une seule discordance n'ouvre PAS une nouvelle scène si le plan suivant correspond à la scène en cours (lookahead)
      - Nouvelle scène uniquement après 2 plans consécutifs qui ne correspondent pas à la scène en cours
    Retourne: liste de tuples (start_ms, end_ms, mid_ms)
    """
    if not shots:
        return []
    # Indexer frames par temps pour retrouver la frame la plus proche du milieu du plan
    frames_sorted = sorted(frames_analysis, key=lambda f: f['time_ms'])
    def mid_frame_signature(start_ms, end_ms, mid_ms):
        # Trouver la frame la plus proche du mid_ms
        # frames_sorted non vide ici (sinon pas d'analyse)
        lo, hi = 0, len(frames_sorted)-1
        best = frames_sorted[0]
        best_d = abs(best['time_ms'] - mid_ms)
        # Recherche linéaire (nb frames faible à interval 2-5s)
        for fr in frames_sorted:
            d = abs(fr['time_ms'] - mid_ms)
            if d < best_d:
                best = fr
                best_d = d
        return _shot_signature_from_frame(best)

    # Signatures des plans
    sigs = []
    for (s_ms, e_ms, m_ms) in shots:
        sigs.append(mid_frame_signature(s_ms, e_ms, m_ms))
    ...
    # Première scène
    cur_start, cur_end, cur_mid = shots[0]
    cur_indices = [0]
    seen_sigs = [sigs[0]]
    consecutive_miss = 0

    def matches_current(idx):
        # même signature exacte
        for ss in seen_sigs:
            if sigs[idx]['objects'] == ss['objects'] and sigs[idx]['persons'] == ss['persons'] and \
               sigs[idx]['h'] == ss['h'] and sigs[idx]['s'] == ss['s'] and sigs[idx]['v'] == ss['v']:
                return True
        # ou similarité suffisante
        for ss in seen_sigs:
            if _signature_similarity(sigs[idx], ss) >= sim_threshold:
                return True
        return False

    for i in range(1, len(shots)):
        s_ms, e_ms, m_ms = shots[i]
        cur_end_candidate = e_ms
        if matches_current(i):
            # Plan conforme à la scène en cours
            cur_end = cur_end_candidate
            cur_indices.append(i)
            seen_sigs.append(sigs[i])
            consecutive_miss = 0
            continue
        # Lookahead: si le prochain correspond, on garde celui-ci dans la scène (pont)
        if i + 1 < len(shots) and matches_current(i+1):
            cur_end = cur_end_candidate
            cur_indices.append(i)
            # ne pas ajouter sa signature pour éviter bruit
            consecutive_miss = 0
            continue
        # Sinon échec
        consecutive_miss += 1
        # Si 2 échecs consécutifs: on clôt la scène précédente à i-1 et on démarre une nouvelle
        if consecutive_miss >= 2:
            # Clôturer la scène
            scenes.append((cur_start, cur_end, (cur_start + cur_end)//2))
            # Nouvelle scène
            cur_start, cur_end, cur_mid = s_ms, e_ms, m_ms
            cur_indices = [i]
            seen_sigs = [sigs[i]]
            consecutive_miss = 0
        else:
            # Tolérer un échec isolé et rester dans la scène
            cur_end = cur_end_candidate
            cur_indices.append(i)
            # pas d'ajout de signature
...
class ScenePatternDetector:
    ...
    def analyze_film(self, video_path, interval_seconds=2, start_time_s=0, end_time_s=None, output_prefix=None):
        ...
        # Détection des plans puis regroupement en scènes (règle MÊME PLAN = MÊME SCÈNE)
        print("Détection des plans…")
        shots = detect_scenes_pyscenedetect(video_path, start_time_s, end_time_s)
        scenes_grouped = group_shots_into_scenes(shots, frames_analysis, sim_threshold=0.5)
        
        timeline = []
        for (s_ms, e_ms, m_ms) in scenes_grouped:
